...
MSet
Enquire::Internal::get_mset(Xapian::doccount first, Xapian::doccount maxitems, MLmodel &ml,
			    ...
			    const MatchDecider *mdecider) const
{
    ...
    if (first_orig != first && retval.internal.get()) {
    	retval.internal->firstitem = first_orig;    // no execute
    }
    ...
    // networked case.
    retval.internal->enquire = this;                // execute

    if (!retval.internal->stats) {
	retval.internal->stats = stats.release();       // execute
    }

    // Second Index From Here
    ml.readFeature(retval);
    ml.getResult();
    ml.readSpyData(spies);
...
MSet
Enquire::get_mset(Xapian::doccount first, Xapian::doccount maxitems, MLmodel &ml,
          Xapian::doccount check_at_least, const RSet *rset,
		  const MatchDecider *mdecider) const
{
    LOGCALL(API, Xapian::MSet, "Xapian::Enquire::get_mset", first | maxitems | check_at_least | rset | mdecider);
    RETURN(internal->get_mset(first, maxitems, ml, check_at_least, rset, mdecider));
}
