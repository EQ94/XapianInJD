...
public:
    ...
    virtual void readFeature(Xapian::MSet mset){
        if(mset.size()>0)
        {
            cout<<"";
        }else{
            cout<<"There are no valid data"<<endl;
        }
    }
    ...
    virtual void readSpyData(const vector<Xapian::Internal::opt_intrusive_ptr<Xapian::MatchSpy> > & matchspies){
    // virtual void readSpyData(const vector<Xapian::Internal::opt_intrusive_ptr<Xapian::ValueCountMatchSpy> > * matchspies){
          cout<<"";
     }

};
/*
class MLmodel2:public MLmodel
{
public:
    MLmodel2(){}
    void ShowResult(Xapian::MSet mset){
        cout<<"MLmldel2 ShowResult()" <<endl;
    }
};
*/
namespace Xapian {
...
class XAPIAN_VISIBILITY_DEFAULT Enquire {
	 ...
	 */
	MSet get_mset(Xapian::doccount first, Xapian::doccount maxitems, MLmodel &ml ,
	...
	MSet get_mset(Xapian::doccount first, Xapian::doccount maxitems, MLmodel &ml,
		      const RSet * omrset,
		      const MatchDecider * mdecider = 0) const {
	    return get_mset(first, maxitems, ml, 0, omrset, mdecider);
	}
